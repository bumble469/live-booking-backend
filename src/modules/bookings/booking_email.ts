...
function buildTicketHtml(args: SendBookingConfirmationArgs): string {
  const { fullname, bookingReference, seatLabels } = args;
        ...
        <table width="520" cellpadding="0" cellspacing="0" style="background:#2A1820;border-radius:12px;overflow:hidden;border:1px solid #4d2f3a;">
          ...
          <tr>
            <td style="padding:28px 36px;">
              <p style="margin:0 0 20px;font-size:15px;color:#a6939a;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0"
                style="border-top:2px dashed #4d2f3a;margin-bottom:24px;position:relative;">
                <tr>
...
export async function sendBookingConfirmation(
  ...
  const { to, fullname, bookingReference, seatLabels } = args;

  const { error } = await resend.emails.send({
    from: 'onboarding@resend.dev',
    to,
    subject: `Your booking is confirmed — ${bookingReference}`,
    html: buildTicketHtml({ to, fullname, bookingReference, seatLabels }),
  });
